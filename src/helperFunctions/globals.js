import React, {useState} from "react"
export const squareSize = 80; 

export const maxSelections = 3;
export const colorBase = ["#0000ff", "#ff0000", "#00ff00", "#6442bd",
                          "#0000ff", "#ff0000", "#00ff00", "#6442bd",
                          "#0000ff", "#ff0000", "#00ff00", "#6442bd",
                          "#0000ff", "#ff0000", "#00ff00", "#6442bd"];
export const playerDesigns = ["japanese-cube", "bricks"];
// export const [difficulty, setDifficulty] = useState(4);
export const maxNoChangeRounds = 3;
export const secondsPerRound = 10;

// export function setFormVals(rows, cols, diff) {
//     numRows = rows;
//     numCols = cols;
//     difficulty = parseInt(diff);
// }
